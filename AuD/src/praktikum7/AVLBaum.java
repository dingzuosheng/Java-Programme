...
// Methoden zum Suchen
	...
	{
	AVLKnoten<T> a=wurzel;
	while(a!=null) {
		if(a.getDaten().compareTo(daten)==0) {
			return true;
		}else {
			if(a.getDaten().compareTo(daten)<0) {
				a=a.getKnotenRechts();
	...
	{ 
		...
		while(!s.isEmpty()) {
			...
			if(k.getKnotenRechts()!=null) {
				s.push(k.getKnotenRechts());
			}
			if(k.getKnotenLinks()!=null) {
				s.push(k.getKnotenLinks());  //warum sollte es die rechteBaum zuerst als linkeBaum gepusht werden ?
			}
