...
// Methoden zum Traversieren
	...
	private String traversierePreOrder(final Knoten<T> einKnoten)
	{
		...
		String erg=einKnoten.getDaten().toString()+",";
		if(einKnoten.getKnotenLinks()!=null) {			
			erg+=traversierePreOrder(einKnoten.getKnotenLinks());	
		}
		if(einKnoten.getKnotenRechts()!=null) {
			erg+=traversierePreOrder(einKnoten.getKnotenRechts());
		}
		return erg;
	}
	...
	private String traversiereInOrder(final Knoten<T> einKnoten)
	{
		...
		String erg="";
		if(einKnoten.getKnotenLinks()!=null) {
			erg+=traversiereInOrder(einKnoten.getKnotenLinks());
		}
		    erg+=einKnoten.getDaten().toString()+",";
		if(einKnoten.getKnotenRechts()!=null) {
			erg+=traversiereInOrder(einKnoten.getKnotenRechts());
		}
	...
	private String traversierePostOrder(final Knoten<T> einKnoten)
	{
        ...
        String erg="";
        if(einKnoten.getKnotenLinks()!=null) {
        	erg+=traversierePostOrder(einKnoten.getKnotenLinks());
        }
        if(einKnoten.getKnotenRechts()!=null) {
        	erg+=traversierePostOrder(einKnoten.getKnotenRechts());
        }
            erg+=einKnoten.getDaten().toString()+",";
